...
namespace ARLocation
{
    ...
    public class Hotspot : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            //log
            Debug.Log($"Hotspot '{gameObject.name}' Update: Distance to position ({state.Position}) = {distance:F2}m, Activation Radius = {HotspotSettings.ActivationRadius:F2}m");
            ...
            if (!state.Activated)
            {
                ...
                {
                    //log
                    Debug.Log($"Hotspot '{gameObject.name}' Update: Within activation radius, attempting to Activate().");
                    Activate();
                }
            }
            else if (HotspotSettings.DeactivateOnLeave)
            {
                if (distance > HotspotSettings.DeactivationRadius)
                {
                    ...
                    //log
                    Debug.Log($"Hotspot '{gameObject.name}' Update: Outside deactivation radius, attempting to Deactivate().");
                    Deactivate();
                }
            }
